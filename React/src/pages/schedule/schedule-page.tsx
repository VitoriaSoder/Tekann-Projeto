...
export default function SchedulePage() {
  ...
  const handleExportCSV = () => {
    const dataToExport = prepareScheduleDataForExport(
      scheduleData.grid,
      selectedDate,
      dateLocale,
    );
    exportToCSV(
      dataToExport,
      `agenda-${format(selectedDate, "EEE, dd MMM", { locale: dateLocale })}`,
    );
  };
  const handlePrev = () => {
  ...
  const renderCourtHeader = (court: ScheduleGridItem) => (
    <div className="flex flex-col items-start text-left">
      <span className="text-sm font-semibold text-foreground tracking-tight leading-none mb-1.5">
        {court.courtName}
      </span>
      ...
      >
        <div className="flex justify-between items-start mb-1.5 md:mb-2 relative z-10 w-full">
          <StatusBadge status={status as any}>
            {status === "OCCUPIED" ? t("schedule:occupied") : t("schedule:available")}
          </StatusBadge>
          {!isOccupied && (
            <ReservationModal
              defaultValues={{
                ...
                startTime: hour,
                endTime: format(
                  addMinutes(new Date(`1970-01-01T${hour}:00`), 60),
                  "HH:mm",
                ),
              }}
              trigger={
  ...
  return (
    <PageLayout
      ...
      descriptionKey={
        isAdmin ? "schedule:description" : "schedule:description_user"
      }
      maxWidth="xl"
    >
      <div className="mb-4 md:mb-6 mt-1 md:mt-2">
        ...
        <div className="hidden md:flex flex-wrap flex-row items-center justify-between gap-4">
            ...
            {viewMode !== "list" && (
              ...
              >
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="h-10 px-4 rounded-full font-medium border-border hover:bg-accent shadow-sm"
                  >
      ...
      <div className="max-w-[1400px] mx-auto w-full anim-fade-up">
        ...
        {viewMode === "list" && (
          <ListView bookings={bookings} selectedDate={selectedDate} />
        )}
      </div>
    </PageLayout>
  );
