...
export function CourtList() {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    {
      header: t("courts:capacity_label"),
      accessorKey: "capacity",
      cell: (item) => `${item.capacity} ${t("courts:people")}`,
    },
    {
      header: t("courts:operation_label"),
      accessorKey: "openingTime",
      cell: (item) => `${item.openingTime} – ${item.closingTime}`,
    },
    {
      ...
      cell: (item) => (
        <StatusBadge status={item.isActive ? "ACTIVE" : "INACTIVE"}>
          <Text
            as="span"
            tKey={
              item.isActive ? "courts:status_active" : "courts:status_inactive"
            }
            variant="inherit"
          />
        </StatusBadge>
