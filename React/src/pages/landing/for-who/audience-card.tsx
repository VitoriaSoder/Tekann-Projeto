...
export function AudienceCard({
  ...
  return (
    <Card
      className={`relative flex flex-col h-full rounded-[2.5rem] border p-12 transition-all duration-500 ${highlight ? "bg-primary text-black border-primary/20 shadow-2xl shadow-primary/20" : "bg-muted/50 text-foreground border-border shadow-sm"}`}
    >
      {highlight && (
      ...
      <CardHeader className="p-0 mb-10 relative z-10">
        <div className="flex items-center gap-6">
          <div
            className={`w-16 h-16 rounded-full flex items-center justify-center ${highlight ? "bg-primary text-black" : "bg-foreground text-background"}`}
          >
            {highlight ? <Building2 size={32} /> : <BookOpen size={32} />}
          </div>
          <Heading
            level={3}
            tKey={badgeKey}
