...
export function ReservationModal({
  ...
  const courts = useAppSelector((s) =>
    s.courts.lista.filter((c) => c.isActive),
  );
      ...
      <DialogTrigger asChild>
        {trigger ? (
          ...
          <Button className="bg-primary text-black font-black uppercase tracking-tight hover:bg-primary/90 rounded-full shadow-sm hover:scale-[1.02] transition-transform h-11 px-6">
            <Text
              variant="none"
              tKey="reservations:new_reservation"
              as="span"
              className="text-sm"
            />
          </Button>
      ...
      <DialogContent className="sm:max-w-[500px] bg-card border border-border rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden p-0 gap-0">
        <Box className="bg-primary/5 px-8 py-6 border-b border-border relative">
          ...
          <DialogTitle className="z-10 relative m-0">
            <Heading
              ...
              tKey={
                isEditing
                  ? "reservations:edit_title"
                  : "reservations:form_title"
              }
              className="text-2xl font-extrabold text-foreground tracking-tight m-0"
            />
          </DialogTitle>
          <DialogDescription className="z-10 relative mt-1">
            <Text
              variant="muted"
              tKey="reservations:form_description"
        ...
        <Form {...form}>
          <form
            ...
            className="px-8 pb-8 pt-6 space-y-5 relative"
          >
            {apiError && (
              <Box className="anim-fade-up bg-destructive/10 border border-destructive/20 text-destructive rounded-2xl p-4 flex items-start gap-3 text-sm font-medium">
                ...
                <Text className="leading-tight text-destructive">
                  {apiError}
                </Text>
              </Box>
            )}
                    ...
                    >
                      <FormControl>
                        <SelectTrigger className="rounded-xl h-12 bg-muted/50 border-border focus:ring-primary focus:border-primary">
                          <SelectValue
                            placeholder={t("reservations:court_placeholder")}
                          />
                        </SelectTrigger>
                      </FormControl>
                ...
                render={({ field }) => (
                  <FormItem className="md:col-span-2">
                    <FormLabel>
                      <Text
                        variant="bold"
                        tKey="reservations:client_label"
                        className="text-foreground text-xs"
                      />
                    </FormLabel>
                    <FormControl>
                      ...
                      <PopoverTrigger asChild>
                        <FormControl>
                          <Button
                            variant={"outline"}
                            className={cn(
                              "rounded-xl h-12 border-border bg-muted/50 pl-3 text-left font-medium hover:bg-transparent  hover:text-foreground w-full",
              ...
              <Box className="md:col-span-2 mt-2">
                ...
                {form.formState.errors.startTime && (
                  <Text
                    ...
                    tKey="reservations:select_time_error"
                  />
                )}
              </Box>
            </Box>
            ...
            <DialogFooter className="pt-8 border-t border-border flex sm:justify-between items-center sm:space-x-3 w-full mt-8">
              <Button
                ...
                className="rounded-full font-medium text-muted-foreground hover:text-foreground w-full sm:w-auto"
              >
                <Text
                  variant="bold"
                  tKey="common:cancel"
